# pages.py  —  ORA v11.1 · Dark Edition
# شامل: LOGO_SVG, BLOOD_HTML, LOGIN_HTML, DASHBOARD_HTML, get_public_page_html()
# v11.1: بخش «آی‌پی‌های تمیز» در تنظیمات + انتخاب Clean IP و حالت مولتی در مودال کانفیگ
# FIX: حذف تعریف تکراری let manageSid که باعث SyntaxError و مرگ کل اسکریپت می‌شد
...
<aside class="sidebar" id="sb">
  <div class="logo">__LOGO_SVG__<div><div class="logo-name">ORA</div><div class="logo-sub">DARK PANEL</div></div><button class="sb-close" id="sbClose"><i class="ti ti-x"></i></button></div>
  <nav class="nav-wrap">
...
<section class="pg on" id="pg-dash">
  ...
  <div class="g3">
    ...
    <div class="card"><div class="card-title"><i class="ti ti-server"></i>وضعیت سیستم</div>
      ...
      <div class="sr"><span class="sr-k"><i class="ti ti-versions"></i>نسخه</span><span class="sr-v">ORA v11.1</span></div>
    </div>
  </div>
...
<section class="pg" id="pg-settings">
  ...
  <div class="g2">
    ...
    <div class="card">
      ...
      <div class="sr" style="margin-top:14px"><span class="sr-k"><i class="ti ti-versions"></i>نسخه</span><span class="sr-v">ORA v11.1 — Dark Edition</span></div>
    </div>
  </div>
</section>
...
<div class="modal" id="mLink"><div class="modal-card">
  ...
  <div class="fgrid">
    ...
    <div class="fg"><label>آی‌پی تمیز (Clean IP)</label><select id="fCleanIp"><option value="">دامنه اصلی (پیش‌فرض)</option></select></div>
...
let page='dash',timers=[],chDash=null,chTraf=null,range='24',lastHourly={},links=[],subs=[],cleanIps=[],confirmCb=null,editId=null,manageSid=null,upSec=null;
...
function renderLinks(){
  ...
  list.innerHTML=items.map(l=>{
    ...
    return '<div class="lcard'+(ok?'':' off')+'">'
      ...
      +(l.multi_ip?'<span class="chip chip-ok"><i class="ti ti-stack-2"></i>مولتی</span>':'')
      +'</div>'
...
function showQrLink(id){const l=findLink(id);if(l)showQr('QR: '+l.label,l.vless_link)}
function copyLinkVless(id){const l=findLink(id);if(l)copyTxt(l.vless_link,'لینک VLESS کپی شد')}
...
function fillCleanIpSelects(){
  const sel=$('#fCleanIp');if(!sel)return;
  const cur=sel.value;
  sel.innerHTML='<option value="">دامنه اصلی (پیش‌فرض)</option>'+cleanIps.map(c=>'<option value="'+esc(c.ip)+'">'+esc(c.ip)+(c.label?' — '+esc(c.label):'')+'</option>').join('');
...
async function editLinkById(id){
  ...
  $('#fNote').value=l.note||'';
  fillCleanIpSelects();
  const csel=$('#fCleanIp'),cv=l.clean_ip||'';
  if(cv&&![...csel.options].some(o=>o.value===cv)){
    const o=document.createElement('option');o.value=cv;o.textContent=cv+' (ذخیره‌شده)';csel.appendChild(o);
  }
  csel.value=cv;
...
async function loadCleanIps(){
  try{const d=await api('/api/clean-ips');cleanIps=d.clean_ips||[];renderCleanIps();fillCleanIpSelects();}catch(e){}
}
